...
resource "google_container_cluster" "primary" {
  ...
  resource_labels = merge(local.mandatory_labels, local.custom_mandatory_labels, var.cluster_resource_labels)

  location          = local.location
  node_locations    = local.node_locations
  cluster_ipv4_cidr = var.cluster_ipv4_cidr == "" ? null : var.cluster_ipv4_cidr
  ...
  master_authorized_networks_config {
    dynamic "cidr_blocks" {
      for_each = var.enable_private_endpoint == true ? var.master_authorized_networks : []
      content {
  ...
  dynamic "private_cluster_config" {
    ...
    content {
      ...
      master_ipv4_cidr_block  = private_cluster_config.value.master_ipv4_cidr_block == null ? "10.0.0.0/28" : private_cluster_config.value.master_ipv4_cidr_block
...
resource "google_container_node_pool" "pools" {
  ...
  lifecycle {
    ignore_changes = [initial_node_count, node_config["labels"], node_config["tags"]]
...
resource "google_compute_network_peering_routes_config" "gke_peering" {
  ...
  count                = var.enable_private_nodes == true ? 1 : 0
